...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2(vp_size.x / 5, vp_size.y - NOTIFY_PADDING_Y - height), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos(vp_size.x / 5.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
				...
				else if (!NOTIFY_NULL_OR_EMPTY(default_title))
				{
					...
					TextColored(text_color, default_title); // Render default title text (ImGuiToastType_Success -> "Success", etc...)
					was_title_rendered = true;
				}
