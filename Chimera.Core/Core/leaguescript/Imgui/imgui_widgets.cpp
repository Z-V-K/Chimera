...
bool ImGui::ButtonEx(const char* key, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImVec2 pos = window->DC.CursorPos + ImVec2(2.f, 0.f);
    ...
    // Render
    const ImU32 col = (held) ? Colors::AccentPrimary : Colors::AccentPrimaryDisabled;
    ...
    PushStyleColor(ImGuiCol_Text, (ImVec4)ImColor(0, 0, 0));
    ...
    IMGUI_TEST_ENGINE_ITEM_INFO(id, key, g.LastItemData.StatusFlags);
    return pressed;
}

bool ImGui::Button(const char* label, const ImVec2& size_arg)
{
    return ButtonEx(label, label, size_arg, ImGuiButtonFlags_None);
}
...
bool ImGui::SmallButton(const char* label)
{
    ...
    bool pressed = ButtonEx(label, label, ImVec2(0, 0), ImGuiButtonFlags_AlignTextBaseLine);
...
{
    return Checkbox(label, label, v);
}

bool ImGui::Checkbox(const char* key, const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImRect check_bb(pos + ImVec2(window->Size.x - width - offset, 0.f), pos + ImVec2(window->Size.x - offset, height));
    const ImRect total_bb(pos, pos + ImVec2(width + width + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), 20.f));

    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id))
    {
        IMGUI_TEST_ENGINE_ITEM_INFO(id, key, g.LastItemData.StatusFlags | ImGuiItemStatusFlags_Checkable | (*v ? ImGuiItemStatusFlags_Checked : 0));
        return false;
    }
    ...
    struct CheckboxAnimation {
        float CirclePos = 6.f;
        ImColor CircleColor = Colors::AccentPrimaryDisabled;
        ImColor FrameColor = Colors::AccentSecondary;
    };
    ...
    TabItemMap->second.CirclePos = ImLerp(TabItemMap->second.CirclePos, *v ? check_bb.GetSize().x - 6.f : 6.f, g.IO.DeltaTime * 12.f);
    TabItemMap->second.CircleColor = ImLerp(TabItemMap->second.CircleColor, *v ? (ImVec4)Colors::AccentPrimary : (ImVec4)Colors::AccentSecondary, g.IO.DeltaTime * 12.f);
    TabItemMap->second.FrameColor = ImLerp(TabItemMap->second.FrameColor, *v ? (ImVec4)Colors::AccentPrimaryDisabled : (ImVec4)Colors::Background, g.IO.DeltaTime * 12.f);

    // checkbox rendering
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min + ImVec2(0, 4.f), check_bb.Max - ImVec2(0, 4.f), TabItemMap->second.FrameColor, false, 12.f);
    window->DrawList->AddCircleFilled(check_bb.Min + ImVec2(TabItemMap->second.CirclePos, check_bb.GetSize().y / 2.f), 6.f, TabItemMap->second.CircleColor);

    ImVec2 label_pos = ImVec2(window->DC.CursorPos.x + 4.f, check_bb.Min.y + 1.f);
...
bool ImGui::BeginCombo(const char* key, const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = 200.f;
    const ImRect bb(cursor_pos + ImVec2(window_size.x - w - 20.f, 0.f), cursor_pos + ImVec2(window_size.x - 16.f, 22.f));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(bb.GetWidth() + label_size.x + 30.f, bb.GetHeight()));

    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &bb))
        return false;

    //window->DrawList->AddRect(total_bb.Min, total_bb.Max, ImColor(255, 0, 255));
    ...
    // Render shape
    const ImU32 frame_col = hovered || popup_open ? Colors::AccentPrimary : Colors::AccentSecondary;
    const float value_x2 = ImMax(bb.Min.x, bb.Max.x);
    RenderNavHighlight(bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRect(bb.Min + ImVec2(0, 1.f), ImVec2(value_x2, bb.Max.y), frame_col, 4.f, ImDrawFlags_RoundCornersAll);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ImU32 bg_col = GetColorU32((popup_open || hovered) ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        //window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, bg_col, 4.f, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        //if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
        RenderArrow(window->DrawList, ImVec2(value_x2 - arrow_size, bb.Min.y + 3.f), text_col, ImGuiDir_Down, 1.0f);
    }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(bb.Min + ImVec2(6.f, 3.f), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    }
    if (label_size.x > 0)
        RenderText(ImVec2(window->DC.CursorPos.x + 4.f, bb.Min.y + 3.f), label);
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    if (!BeginCombo(label, label, preview_value, ImGuiComboFlags_None))
        return false;
...
bool ImGui::SliderScalar(const char* key, const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 cursor_pos = window->DC.CursorPos + ImVec2(4.f, 0.f);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect label_pos(cursor_pos, cursor_pos + label_size + ImVec2(10.f, 0.f));
    const float window_size = window->Size.x;
    const float slider_width = 120.f;
    char buff[64];
    sprintf(buff, format, p_min);
    const float value_width = 100.f;

    //const float value_width = data_type == ImGuiDataType_Float ? 80 : 60;
    const ImRect frame_pos(cursor_pos + ImVec2(window_size - slider_width - value_width - 20.f, 0.f), cursor_pos + ImVec2(window_size - 4.f, 15.f));
    const ImRect slider_pos(frame_pos.Min + ImVec2(0, 2.f), frame_pos.Min + ImVec2(slider_width, 15.f));
    const ImRect value_pos(ImVec2(slider_pos.Max.x, slider_pos.Min.y), slider_pos.Max + ImVec2(value_width, 0.f));
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(frame_pos.GetWidth() + label_pos.GetWidth(), 20.f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(total_bb, style.FramePadding.y);
    if (!ItemAdd(total_bb, id, &slider_pos, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;

    //window->DrawList->AddRect(total_bb.Min, total_bb.Max, ImColor(255, 255, 0));
    ...
    const bool hovered = ItemHoverable(slider_pos, id);
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
    ...
    if (/*temp_input_is_active*/ false)
    {
        ...
        return TempInputScalar(slider_pos, id, key, data_type, p_data, format, is_clamp_input ? p_min : NULL, is_clamp_input ? p_max : NULL);
    }

    struct SliderAnimation {
        float inner_circle_size = 3.f;
    };
    static std::map<ImGuiID, SliderAnimation> sliderAnimations;
    auto animHandle = sliderAnimations.find(ImColor());

    if (animHandle == sliderAnimations.end())
    {
        sliderAnimations.insert({ ImGui::GetItemID(), SliderAnimation() });
        animHandle = sliderAnimations.find(ImGui::GetItemID());
    }

    // Draw frame
    const ImU32 frame_col = Colors::Background;
    RenderNavHighlight(slider_pos, id);
    RenderFrame(slider_pos.Min + ImVec2(3.f, 5.f), slider_pos.Max - ImVec2(3.f, 5.f), frame_col, true, 8.f);

    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(slider_pos, id, data_type, p_data, p_min, p_max, format, flags, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    {
        const bool grab_hovered = ItemHoverable(grab_bb, id);
        animHandle->second.inner_circle_size = ImLerp(animHandle->second.inner_circle_size, grab_hovered ? 4.f : 3.f, g.IO.DeltaTime * 20.f);

        window->DrawList->AddRectFilled(slider_pos.Min + ImVec2(2.f, 5.f), grab_bb.GetCenter() + ImVec2(0, 2.5f), Colors::AccentPrimaryDisabled, 8.f);
        window->DrawList->AddCircleFilled((grab_bb.Min + grab_bb.Max) / 2, 6.f, Colors::AccentPrimary);
        //window->DrawList->AddCircleFilled((grab_bb.Min + grab_bb.Max) / 2, animHandle->second.inner_circle_size, Colors::AccentPrimary);
    }
    ...
    window->DrawList->AddRectFilled(value_pos.Min, value_pos.Max, Colors::Background, 4.f);
    PushFont(GetIO().Fonts->Fonts[1]);
    RenderTextClipped(value_pos.Min, value_pos.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    PopFont();

    if (label_size.x > 0.0f)
        RenderText(ImVec2(window->DC.CursorPos.x + 4.f, total_bb.Min.y + 1.f), label);
...
{
    return SliderScalar(label, label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::SliderFloat(const char* key, const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(key, label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::SliderInt(const char* key, const char* label, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return SliderScalar(key, label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        RenderFrame(frame_bb.Min, frame_bb.Max, Colors::Background, true, style.FrameRounding);
    }
...
{
    return ColorEdit4(label, label, col, flags | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(const char* key, const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            if (ColorEdit4("##rgb", "##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB))
            {
      ...
      /*  if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4("##hsv", "##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags_DisplayMask_) == 0)
            value_changed |= ColorEdit4("##hex", "##hex", col, sub_flags | ImGuiColorEditFlags_DisplayHex);*/
        PopItemWidth();
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (hovered || selected)
    {
        const ImU32 col = Colors::AccentSecondary;
        RenderFrame(bb.Min, bb.Max, col, false, 4.0f);
    }
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    else
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        pressed = Selectable("", menu_is_open, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 15.f));
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
            RenderText(pos + ImVec2(offsets->OffsetIcon, 0.0f), icon);
        RenderArrow(window->DrawList, pos + ImVec2(offsets->OffsetMark + extra_w + g.FontSize * 0.30f, -0.f), GetColorU32(ImGuiCol_Text), ImGuiDir_Right);
    }
    if (!enabled)
        EndDisabled();
    ...
    if (want_open && !menu_is_open && g.OpenPopupStack.Size > g.BeginPopupStack.Size)
    {
        ...
        OpenPopup(label, ImGuiPopupFlags_BeginMenu);
    }
    else if (want_open)
    {
        menu_is_open = true;
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", "##v", NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
...
bool ImGui::Hotkey(const char* widget_key, const char* label, int* k, bool* hold, const ImVec2& size_arg)
{
    ...
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImGuiID id = window->GetID(widget_key);
    const ImVec2 label_size = ImGui::CalcTextSize(label, NULL, true);
    ImVec2 size = ImGui::CalcItemSize(size_arg, ImGui::CalcItemWidth(), label_size.y + style.FramePadding.y * 2.0f);
    const ImRect frame_bb(pos + ImVec2(square_sz2 - 70.f, 0.f), pos + ImVec2(square_sz2 - 2.f, 20.f));
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), 20.f));
    ...
    TabItemMap->second.Frame = ImLerp(TabItemMap->second.Frame, g.ActiveId == id ? (ImVec4)Colors::AccentPrimary : (ImVec4)Colors::AccentPrimaryDisabled, g.IO.DeltaTime * 12.f);;
    //if (arrow_hovered && io.MouseClicked[0])
    //    display_popup = true;
    ...
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, TabItemMap->second.Frame, true, 3.f);
    ...
    if (label_size.x > 0)
    {
        ImVec2 label_pos = ImVec2(total_bb.Min.x + 4.f, total_bb.GetCenter().y - CalcTextSize(label).y / 2.f);
        window->DrawList->AddText(label_pos, ImColor(255, 255, 255), label);
    }

    return value_changed;
}

void ImGui::Separator(const char* key, const char* label)
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const ImVec2 label_size = label == "" ? ImVec2(0, 0) : CalcTextSize(label);
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(label_size.x, 20.f));

    ItemSize(total_bb, style.FramePadding.y);
    if (!ImGui::ItemAdd(total_bb, id))
        return;

    const float window_width = ImGui::GetWindowSize().x - 10.f;
    const float margin = label == "" ? 0.f : 6.f;

    window->DrawList->AddLine(pos + ImVec2(0, 12.f), pos + ImVec2(((window_width - label_size.x) / 2) - margin, 12.f), ImColor(255, 255, 255));
    window->DrawList->AddLine(pos + ImVec2(((window_width - label_size.x) / 2) + label_size.x + margin, 12.f), pos + ImVec2(window_width - 6.f, 12.f), ImColor(255, 255, 255));
    window->DrawList->AddText(pos + ImVec2((window_width - label_size.x) / 2, 4.f), ImColor(255, 255, 255), label);
}

void ImGui::SameLineColorEdit(const char* key, float col[4], int type)
{
    //ImGuiWindow* window = GetCurrentWindow();
    //if (window->SkipItems)
    //    return;

    //const auto flags = ImGuiColorEditFlags_PickerHueWheel | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoTooltip | ImGuiColorEditFlags_NoDragDrop | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_NoOptions;
    ...
    ////if (type != (int)Script::Native::EntryType::key)
    //ImGui::SameLine();

    //const float square_sz2 = ImGui::GetColumnWidth();

    //const float w_full = ImGui::GetColumnWidth();
    //const float w_button = (flags & ImGuiColorEditFlags_NoSmallPreview) ? 0.0f : (square_sz + style.ItemInnerSpacing.x);
    //const float w_inputs = w_full - w_button;

    //const ImVec2 pos = window->DC.CursorPos;

    //float offset = type == EntryType::checkbox ? 50.f : 220.f;

    ////switch (type)
    ////{
    ////case (int)EntryType::checkbox:
    ////    offset = 60.f;
    ////    break;
    ////case (int)EntryType::key:
    ////    offset = 30.f;
    ////    break;
    ////}

    //const ImRect preview_bb(pos + ImVec2(square_sz2 - offset, 2.f), pos + ImVec2(square_sz2 - 10.f, 0.f));

    //window->DC.CursorPos = preview_bb.Min;

    //std::string total_key = key + std::string("##ColorButton");
    //PushID(total_key.c_str());

    //if (ColorButton(total_key.c_str(), col_v4, flags, ImVec2(16, 16)))
    //{
    //    if (!(flags & ImGuiColorEditFlags_NoPicker))
    //    {
    //        // Store current color and open a picker
